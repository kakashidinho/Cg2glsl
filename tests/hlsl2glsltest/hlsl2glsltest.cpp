...
struct glslopt_ctx * g_glsl_optContext[2];
...
static std::string optimizeGLSL(const char *source, 
							EShLanguage language,
							ETargetVersion version)
{
	glslopt_ctx*  glsl_optContext = version == ETargetGLSL_ES_100 ? g_glsl_optContext[1] : g_glsl_optContext[0];
	const glslopt_shader_type shaderType = (language == EShLangVertex) ? kGlslOptShaderVertex : kGlslOptShaderFragment;
					  ...
					  bool optimize = false)
{
	...
	if (parseOk)
	{
		...
		if (translateOk)
		{
			...
			if (optimize)
			{
				std::string opt_text = optimizeGLSL(text.c_str(), kTypeLangs[type], version);
				if (opt_text.size() > 0)
					text = opt_text;
			}
	...
	int cout = Hlsl2Glsl_GetUniformCount(parser);
	const ShUniformInfo* info = Hlsl2Glsl_GetUniformInfo(parser);
					  ...
					  bool checkGL)
{
	...
	if (type == VERTEX_FAILURES || type == FRAGMENT_FAILURES) {
		...
		return TestFile(type, inputPath, outname + suffix, "main", NULL,version, options, checkGL);
	}
	return false;
}
...
#	endif
	...
	glslopt_cleanup(g_glsl_optContext[0]);
	glslopt_cleanup(g_glsl_optContext[1]);
