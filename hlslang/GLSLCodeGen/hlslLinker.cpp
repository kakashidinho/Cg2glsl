...
static inline bool IsMultipleFunctionFound(int result)
{
	return result > 1 || result < -1;
}

static inline bool IsFunctionFound(int result)
{
	return result != 0;
}
								...
								)
{
	...
	else if (functionMatchRe == FUNC_NAME_MATCH)
	{
		...
		else if (result <= -1)//more than 1 function found
			result--;
	}
}
...
bool HlslLinker::addCalledFunctions( GlslFunction *func, FunctionSet& funcSet, std::vector<GlslFunction*> &funcList )
{
	...
	for (std::set<std::string>::const_iterator cit=cf.begin(); cit != cf.end(); cit++)
	{
		GlslFunction* func = NULL;
		{
			...
			if (std::find (funcSet.begin(), funcSet.end(), func) == funcSet.end())
				funcSet.push_back (func);
		}
		addCalledFunctions( func, funcSet, funcList); 
	}
...
bool HlslLinker::buildFunctionLists(HlslCrossCompiler* comp, const std::string& entryPoint, GlslFunction*& globalFunction, std::vector<GlslFunction*>& functionList, FunctionSet& calledFunctions, GlslFunction*& funcMain)
{
	...
	if (IsMultipleFunctionFound(result))
	{
...
bool HlslLinker::link(HlslCrossCompiler* compiler, const char* entryFunc, const char* profile, ETargetVersion targetVersion, unsigned options)
{
	...
	// revert functions to their normal names (without profile prefix)
	for (std::vector<GlslFunction*>::iterator ite = calledFunctions.begin(); 
	...
	for (int ii=0; ii<pCount; ii++)
	{
		...
		case EqtUniform:
			uniform << "uniform ";
			if (sym->isNonSquareMatrix())
			{
				writeType (uniform, (EGlslSymbolType)(EgstFloat2 + (sym->getNonSquareRows() - 2)), NULL, usePrecision?sym->getPrecision():EbpUndefined);
			}
			else
				writeType (uniform, sym->getType(), sym->getStruct(), usePrecision?sym->getPrecision():EbpUndefined);
			//uniform << " xlu_" << sym->getName();
			uniform << " " << sym->getName();
			if(sym->getArraySize())
			{
				if (sym->isNonSquareMatrix())
					uniform << "[" << sym->getNonSquareColumns() * sym->getArraySize() << "]";
				else
					uniform << "[" << sym->getArraySize() << "]";
			}
			else if (sym->isNonSquareMatrix())
				uniform << "[" << sym->getNonSquareColumns() << "]";
			uniform << ";\n";
