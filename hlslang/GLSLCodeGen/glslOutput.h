...
public:
	TGlslOutputTraverser (TInfoSink& i, 
	...
	//temporary variable returns from last traversal, useful if one rhs transformed into multiple statement
	std::string tempVariableName;
	//parent node requires a value
	bool parentRequireValue;
	//inline expansion process
	bool isInlining;
	std::map<int, std::string>* inlineParamsMap;
	std::string *inlineRetVar;//temporary variable that hold returning value
	std::map<TString, TIntermAggregate*> &inlinefuncList;
