...
bool TParseContext::constructorErrorCheck(const TSourceLoc& line, TIntermNode* node, TFunction& function, TOperator op, TType* type)
{
   *type = function.getReturnType();
   ...
   if (!type->isNonSquareMatrix() && op == EOpConstructStruct && !type->isArray() && (type->getStruct()->size() != function.getParamCount() && function.getParamCount() != 1))
   {
...
TIntermTyped* TParseContext::addConstructor(TIntermNode* node, const TType* type, TOperator op, TFunction* fnCall, TSourceLoc line)
{
	...
	if (node->getAsAggregate()) {
		...
		else if (type->isNonSquareMatrix() && params.size() != type->getNonSquareColumns())
		{
			return constructNonSQuareMatrix(type, node, line);
		}
	...
	else if (type->isNonSquareMatrix())
		return constructNonSQuareMatrix(type, node, node->getLine());
	else if (op == EOpConstructStruct)
...
TIntermTyped* TParseContext::constructBuiltIn(const TType* type, TOperator op, TIntermNode* node, TSourceLoc line, bool subset)
{
	...
	if (newNode == 0)
	{
		TIntermTyped *node_type = node->getAsTyped();
		
		//check for special case: convert non square matrix to square matrix
		if ((op == EOpConstructMat2 || op == EOpConstructMat3) && node_type!= NULL && 
			node_type->isNonSquareMatrix())
		{
			//non-square matrix size must be larger than destination matrix size
			if (op == EOpConstructMat2 
				&& node_type->getTypePointer()->getNonSquareColumns() >= 2 
				&& node_type->getTypePointer()->getNonSquareRows() >= 2)
				newNode = node_type;
			if (op == EOpConstructMat3 
				&& node_type->getTypePointer()->getNonSquareColumns() >= 3 
				&& node_type->getTypePointer()->getNonSquareRows() >= 3)
				newNode = node_type;
		}
	}
	...
	if ( type->isMatrix())
	{
	  bool isRHNonSquareMatrix = false;
	  bool isRHMatrix = newNode->getTypePointer()->isMatrix();
	  if (!isRHMatrix)
			isRHNonSquareMatrix = newNode->getTypePointer()->isNonSquareMatrix();
	  switch (type->getNominalSize())
	  {
	  case 2:
		 if (isRHMatrix)
		 ...
		 {
			 specialName = "__constructfloat2x2(";
			 specialName += newNode->getTypePointer()->getMangledName();
			 op = EOpConstructMat2FromNonSquareMat;
		 }
		 break;
	  case 3:
		 if (isRHMatrix)
		 ...
		 {
			 specialName = "__constructfloat3x3(";
			 specialName += newNode->getTypePointer()->getMangledName();
			 op = EOpConstructMat3FromNonSquareMat;
		 }
		 break;
	  case 4:
	...
	TIntermAggregate *re = intermediate.setAggregateOperator(newNode, op, line);
	if (specialName.size() > 0 )
	{
		re->setName(specialName);
	}
...
TIntermTyped* TParseContext::constructNonSQuareMatrix(const TType* type, TIntermNode* node, TSourceLoc line)
{
	TIntermAggregate* newNode = NULL;
	...
	newNode = intermediate.setAggregateOperator(NULL, EOpFunctionCall, line);

	TString funcName = "__construct";
	funcName += type->getTypeName();

	newNode->setPlainName(funcName);
	newNode->setType(*type);
	...
	if (node->getAsAggregate())
	{
		...
		if (sequence.size() > type->getObjectSize())
		{
			error(line, "too many data provided for construction", "constructor", "");
			return 0;
		}
		else if (sequence.size() < type->getObjectSize())
		{
			error(line, "not enough data provided for construction", "constructor", "");
			return 0;
		}

		for (TIntermSequence::iterator ite = sequence.begin();
			ite != sequence.end() ;
			++ite)
		{
			//try to convert all to floats
			TIntermTyped *child = NULL;
			if (!(*ite)->getAsTyped() || (child = (*ite)->getAsTyped())->isArray())
			{
				error((*ite)->getLine(), "can't convert", "constructor", "");
				return 0;
			}

			child = intermediate.addConversion(EOpConstructFloat, TType(EbtFloat, EbpUndefined), child);
			if (child == NULL)
			{
				error((*ite)->getLine(), "can't convert", "constructor", "");
				return 0;
			...
			newNode->getSequence().push_back(child);

			funcName += child->getTypePointer()->getMangledName();
	...
	else if (node->getAsTyped())//scalar or matrix
	{
		//must be cast down from larger matrix
		TIntermTyped *node_type = node->getAsTyped();
		if (node_type->isNonSquareMatrix())
		{
			if (node_type->getTypePointer()->getNonSquareColumns() < type->getNonSquareColumns()
				|| node_type->getTypePointer()->getNonSquareRows() < type->getNonSquareRows())
			{
				error(line, "can't convert", "constructor", "");
				return 0;
			}
		}
		else if (node_type->isMatrix())
		{
			if (node_type->getNominalSize() < type->getNonSquareColumns()
				|| node_type->getNominalSize() < type->getNonSquareRows())
			{
				error(line, "can't convert", "constructor", "");
				return 0;
			}
		}
		else//scalar conversion to float
		{
			node_type = intermediate.addConversion(EOpConstructFloat, TType(EbtFloat, EbpUndefined), node_type);
			if (!node_type)
			{
				error(line, "can't convert", "constructor", "");
				return 0;
		...
		funcName += node_type->getTypePointer()->getMangledName();

		newNode->getSequence().push_back(node_type);
	}
	else
	{
		error(line, "constructor argument does not have a type", "constructor", "");
	...
	newNode->setName(funcName);//mangled name
