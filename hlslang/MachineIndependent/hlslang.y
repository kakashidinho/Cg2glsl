...
postfix_expression
    ...
    | postfix_expression LEFT_BRACKET expression RIGHT_BRACKET {
        ...
        if (exprNode->getBasicType() != EbtInt || exprNode->getNominalSize() != 1)
        {
			...
			if (!newNode)
			{
				parseContext.error($2.line, "cannot convert to integer", "[]", "");
				parseContext.recover();
			}
			else
				exprNode = newNode;
        ...
        bool isNonSquareMatrix =  $1->isNonSquareMatrix();
        if (!$1->isArray() && !$1->isMatrix() && !$1->isVector() && !isNonSquareMatrix) {
		...
		if (exprNode->getQualifier() == EvqConst) {
			if (((($1->isVector() || $1->isMatrix()) && $1->getType().getNominalSize() <= exprNode->getAsConstant()->toInt()) 
			    || (isNonSquareMatrix && $1->getType().getNonSquareColumns() <= exprNode->getAsConstant()->toInt())) 
			    && !$1->isArray() ) {
				parseContext.error($2.line, "", "[", "field selection out of range '%d'", exprNode->getAsConstant()->toInt());
				parseContext.recover();
			} else {
				if ($1->isArray()) {
					if ($1->getType().getArraySize() == 0) {
						if ($1->getType().getMaxArraySize() <= exprNode->getAsConstant()->toInt()) {
							if (parseContext.arraySetMaxSize($1->getAsSymbolNode(), $1->getTypePointer(), exprNode->getAsConstant()->toInt(), true, $2.line))
								parseContext.recover(); 
						} else {
							if (parseContext.arraySetMaxSize($1->getAsSymbolNode(), $1->getTypePointer(), 0, false, $2.line))
								parseContext.recover(); 
						}
					} else if ( exprNode->getAsConstant()->toInt() >= $1->getType().getArraySize()) {
						parseContext.error($2.line, "", "[", "array index out of range '%d'", exprNode->getAsConstant()->toInt());
						parseContext.recover();
					}
				}
				if (isNonSquareMatrix && !$1->isArray())
				{
					unsigned int i = $1->getType().getNonSquareFieldIndex(exprNode->getAsConstant()->toInt());
					TTypeList* fields = $1->getType().getStruct();
					TIntermConstant* index = parseContext.intermediate.addConstant(TType(EbtInt, EbpUndefined, EvqConst), $2.line);
					index->setValue(i);
					$$ = parseContext.intermediate.addIndex(EOpIndexDirectStruct, $1, index, $2.line);                
					$$->setType(*(*fields)[i].type);
				}
				else
					$$ = parseContext.intermediate.addIndex(EOpIndexDirect, $1, exprNode, $2.line);
			}
		} else {
			...
			if (isNonSquareMatrix && !$1->isArray())
			{
				parseContext.error($2.line, "", "[", "field index of non square matrix must be constant");
				parseContext.recover();
			}
			
			$$ = parseContext.intermediate.addIndex(EOpIndexIndirect, $1, exprNode, $2.line);
		}
        if ($$ == 0) {
...
function_header 
    ...
    | PROFILE fully_specified_type IDENTIFIER LEFT_PAREN {
        ...
        // make sure a sampler is not involved as well...
        if (parseContext.structQualifierErrorCheck($3.line, $2))
            parseContext.recover();
		...
		if (!IsSampler($1.type)) {
			...
			if (!parseContext.executeInitializer($2.line, *$2.string, $3, $1, $5, symbol)) {
				if (symbol)
				{
					...
					if ($$ == 0)//for some reasons, cannot use initializer on declaration
						parseContext.recover();
				}	
				else
					$$ = 0;
...
function_definition
    ...
    compound_statement_no_new_scope {
        ...
        parseContext.symbolTable.pop();
        TIntermAggregate *newNode = parseContext.intermediate.growAggregate($1.intermAggregate, $3, gNullSourceLoc);
        $$ = newNode;
        ...
        if ($1.function->isInline())
			parseContext.inlineFuncList[$1.function->getMangledName()] = newNode;
