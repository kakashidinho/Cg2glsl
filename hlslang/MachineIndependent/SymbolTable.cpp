...
bool TType::isNonSquareMatrix() const
{
	...
	{
		const TString& l_typeName = getTypeName();
...
int TType::getNonSquareRows() const
{
	if (!isNonSquareMatrix())
		return getNominalSize();

	return getStruct()->front().type->getNominalSize();
}

TBasicType TType::getNonSquareMatBasicType() const
{
	if (isNonSquareMatrix())
		return EbtFloat;
...
int TType::getNonSquareFieldIndex(int array_subscriptIndex) const
{
	if (getStruct() == NULL)
		return -1;

	TTypeList* fields = getStruct();
	TString fieldName = "col";
	fieldName += ('0' + array_subscriptIndex);
	for (int i = 0; i < fields->size(); ++i) {
		if ((*fields)[i].type->getFieldName() == fieldName) {
